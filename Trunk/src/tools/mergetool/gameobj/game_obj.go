...
func NewGameObjectFromBinary(data []byte) *GameObject {
	...
	if compress {
		...
		ar := utils.NewLoadArchiver(out.Bytes())
		g.Load(ar)
	} else {
...
func MustRead(ar *utils.LoadArchive, val interface{}) {
	err := ar.Read(val)
	if err != nil {
		panic(err)
	}
...
func (g *GameObject) Load(ar *utils.LoadArchive) {
...
func (g *GameObject) LoadAttr(ar *utils.LoadArchive, attrcount int32) {
...
func (g *GameObject) LoadRecord(ar *utils.LoadArchive, reccount int32) {
...
func (g *GameObject) Store(ar *utils.StoreArchive) error {
...
func (g *GameObject) StoreAttr(ar *utils.StoreArchive) error {
	size := g.Attrs.Count()
	for i := 0; i < size; i++ {
...
func (g *GameObject) StoreRecord(ar *utils.StoreArchive) error {
	size := g.Records.Size()
	for i := 0; i < size; i++ {
