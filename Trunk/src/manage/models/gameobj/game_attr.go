...
func (a *Any) Double() float64 {
	if a.typ == protocol.VTYPE_DOUBLE {
...
func (a *Any) String() string {
	if a.typ == protocol.VTYPE_STRING {
		return a.Val.(string)
	}
	return ""
}
...
func (a *Any) ObjectId() uint64 {
	if a.typ == protocol.VTYPE_OBJECT {
		return a.Val.(uint64)
	}
	return 0
}
...
func (a *Any) FromString(s string) error {
	...
	case protocol.VTYPE_INT:
		var val int32
		if err := toolkit.ParseStrNumber(s, &val); err != nil {
			return err
		}
		a.Val = val
	case protocol.VTYPE_INT64:
