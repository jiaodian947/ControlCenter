...
// Encode string to md5 16 bits hex value
func EncodeMd5ToShort(str string) string {
	m := md5.New()
	m.Write([]byte(str))
	return hex.EncodeToString(m.Sum(nil)[4:12])
}
...
func VerifyToken(account string, token string) bool {
	code, err := hex.DecodeString(token)
	if err != nil {
		return false
	}
	starttime := binary.LittleEndian.Uint64(code[:8])
	lifetime := binary.LittleEndian.Uint16(code[8:12])
	tmp := CreateToken(account, lifetime, starttime)
	if token[28:] != tmp[28:] {
		return false
	}

	before := time.Unix(int64(starttime), 0)
	if before.Add(time.Minute*time.Duration(lifetime)).Unix() < time.Now().Unix() {
		return false
	}
