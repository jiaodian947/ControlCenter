...
func RegisterUser(user *models.User, username, nickname, email, password string) error {
...
func VerifyUser(user *models.User, username, password string) (success bool) {
	...
	if HasUser(user, username) == false {
		beego.Info("user not found", username, password)
		return
	}
	...
	if VerifyPassword(password, user.Password) {
		...
		beego.Info("verify error", username, password)
	}
...
func VerifyUserWithCDKey(user *models.User, cdkey string) bool {
	ac := new(models.ActivationCode)
	ac.Code = cdkey
	if err := ac.Read("code"); err != nil {
		return false
	}
